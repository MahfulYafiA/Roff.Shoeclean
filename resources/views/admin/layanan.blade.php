...
<head>
    ...
    <title>Manajemen Layanan - ROFF.ADMIN</title>
...
@php
    // KHUSUS ADMIN MENGGUNAKAN TEMA BIRU
    $accent = 'blue';

    // AMBIL DATA DARI DATABASE (ms_pengaturan)
...
<body class="text-slate-200 antialiased flex h-screen overflow-hidden relative selection:bg-blue-500 selection:text-white">
    ...
    <main class="flex-1 flex flex-col min-w-0 bg-[#0f172a] relative z-10 h-screen">
        ...
        <header class="bg-[#0f172a]/40 backdrop-blur-xl border-b border-white/5 px-6 md:px-12 py-4 flex justify-between items-center shrink-0 z-40">
            <div class="flex items-center gap-3 md:gap-4">
                <a href="{{ route('admin.dashboard') }}" class="w-9 h-9 md:w-10 md:h-10 rounded-full bg-slate-800/50 border border-slate-700 text-slate-400 hover:text-blue-400 hover:bg-slate-800 transition-all flex items-center justify-center shadow-sm group active:scale-95">
        ...
        <div class="p-6 md:p-12 flex-1 overflow-y-auto custom-scroll relative">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10 relative z-10">
                <div>
                    <div class="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 px-4 py-1.5 rounded-full mb-4">
                        <span class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse shadow-[0_0_10px_currentColor]"></span>
                        <p class="text-[8px] md:text-[9px] font-black text-blue-400 uppercase tracking-[0.4em]">Service Management</p>
                    </div>
                    ...
                    <p class="text-slate-400 font-medium text-sm">Kelola daftar harga dan deskripsi layanan ROFF</p>
                </div>
            ...
            {{-- SECTION PENGATURAN GAMBAR (KEMBALI HADIR UNTUK ADMIN) --}}
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12 relative z-10">
                ...
                <div class="glass-panel rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group border border-white/5">
                    <div class="flex items-center gap-4 mb-8">
                        <div class="w-12 h-12 bg-slate-800 text-blue-400 rounded-xl flex items-center justify-center text-xl border border-slate-700 shadow-lg"><i class="fa-solid fa-image"></i></div>
                    ...
                    <form action="{{ route('update.hero.universal') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex items-center justify-around gap-4 bg-slate-900/50 p-6 rounded-[2rem] border border-slate-700 shadow-inner mb-6">
                            ...
                            <div class="text-center space-y-3">
                                <div class="w-28 h-20 rounded-xl border-2 border-dashed border-blue-500/50 overflow-hidden bg-slate-800 flex items-center justify-center">
                                    <img id="preview_banner_img" src="" class="hidden w-full h-full object-cover">
                                    <i id="placeholder_banner" class="fa-solid fa-cloud-arrow-up text-blue-500/20 text-xl"></i>
                                </div>
                                <p class="text-[8px] font-black text-blue-500 uppercase tracking-widest">Baru</p>
                            </div>
                        </div>
                        <div class="space-y-4">
                            <input type="file" name="hero_image" required class="w-full bg-slate-800 border border-slate-700 px-5 py-3 rounded-2xl text-[10px] text-slate-400 file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-blue-600 file:text-white" onchange="previewFile(event, 'preview_banner_img', 'placeholder_banner')">
                            <button type="submit" class="w-full bg-slate-800 border border-slate-700 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-blue-600 transition-all shadow-xl">Update Banner</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="glass-panel rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group border border-white/5">
                    <div class="flex items-center gap-4 mb-8">
                        <div class="w-12 h-12 bg-slate-800 text-blue-400 rounded-xl flex items-center justify-center text-xl border border-slate-700 shadow-lg"><i class="fa-solid fa-users"></i></div>
                        ...
                        <div class="flex items-center justify-around gap-4 bg-slate-900/50 p-6 rounded-[2rem] border border-slate-700 shadow-inner mb-6">
                            ...
                            <div class="text-center space-y-3">
                                <div class="w-28 h-20 rounded-xl border-2 border-dashed border-blue-500/50 overflow-hidden bg-slate-800 flex items-center justify-center">
                                    ...
                                    <i id="placeholder_tentang" class="fa-solid fa-cloud-arrow-up text-blue-500/20 text-xl"></i>
                                </div>
                                <p class="text-[8px] font-black text-blue-500 uppercase tracking-widest">Baru</p>
                            </div>
                        </div>
                        <div class="space-y-4">
                            <input type="file" name="tentang_image" required class="w-full bg-slate-800 border border-slate-700 px-5 py-3 rounded-2xl text-[10px] text-slate-400 file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-blue-600 file:text-white" onchange="previewFile(event, 'preview_tentang_img', 'placeholder_tentang')">
                            <button type="submit" class="w-full bg-slate-800 border border-slate-700 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-blue-600 transition-all shadow-xl">Update Foto Profil</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="overflow-x-auto pb-4">
                    <table class="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody class="divide-y divide-slate-800/50 text-sm relative">
                            ...
                            <tr class="hover:bg-slate-800/30 transition-all group {{ $l->status == 'Aktif' ? '' : 'opacity-50 grayscale hover:grayscale-0 hover:opacity-100' }}">
                                ...
                                <td class="px-8 py-5 text-center">
                                    @if($l->status == 'Aktif')
                                        <span class="bg-blue-500/20 text-blue-400 px-4 py-1.5 rounded-full text-[9px] font-black uppercase border border-blue-500/30 shadow-inner">Aktif</span>
                                    @else
                                        <span class="bg-slate-700 text-slate-400 px-4 py-1.5 rounded-full text-[9px] font-black uppercase border border-slate-600 shadow-inner">Nonaktif</span>
                                    @endif
                                </td>
                                <td class="px-8 py-5 font-black text-blue-400 text-lg tracking-tighter">Rp {{ number_format($l->harga, 0, ',', '.') }}</td>
                                <td class="px-8 py-5 text-right whitespace-nowrap">
                                    <div class="flex justify-end gap-3">
                                        <form action="{{ route('admin.layanan.toggle', $l->id_layanan) }}" method="POST" class="m-0 inline-block">
                                            @csrf
                                            <button type="submit" class="w-11 h-11 rounded-full transition-all flex items-center justify-center shadow-lg active:scale-90 border border-slate-700 hover:border-transparent {{ $l->status == 'Aktif' ? 'bg-slate-800 text-blue-400 hover:bg-blue-500 hover:text-white' : 'bg-slate-800/50 text-slate-500 hover:bg-slate-600 hover:text-white' }}">
                                                <i class="fa-solid {{ $l->status == 'Aktif' ? 'fa-toggle-on' : 'fa-toggle-off' }} text-base"></i>
                                            </button>
                                        </form>
                                        <button onclick='openEditModal({{ $l->id_layanan }}, "{{ $l->nama_layanan }}", "{{ $l->deskripsi }}", {{ $l->harga }}, "{{ $l->gambar ? asset('storage/' . $l->gambar) : '' }}")' class="w-11 h-11 rounded-full bg-slate-800 text-slate-400 hover:bg-blue-500 hover:text-white border border-slate-700 transition-all flex items-center justify-center shadow-lg hover:scale-110 active:scale-95">
                                            <i class="fa-solid fa-pen-to-square text-sm"></i>
                                        </button>
                                        <form action="{{ route('admin.layanan.destroy', $l->id_layanan) }}" method="POST" onsubmit="return confirm('Hapus layanan?');" class="m-0 inline-block">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="p-24 text-center opacity-30 italic text-sm">
                                    <div class="flex flex-col items-center">
                                        <i class="fa-solid fa-tags text-5xl mb-4 text-slate-400"></i>
            ...
            <div class="mt-auto pt-6 pb-2 border-t border-white/5 flex justify-center items-center opacity-40 shrink-0 relative z-10">
                <p class="text-[9px] font-black uppercase tracking-[0.2em] w-full text-center text-white">© 2026 ROFF.STAFF PANEL CONTROL</p>
            </div>
        </div>
    </main>

    {{-- MODAL EDIT LAYANAN (MODERN 2-COLUMN UNTUK ADMIN) --}}
    <div id="modalEdit" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-5xl bg-slate-900 border border-slate-700 rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.8)] overflow-hidden">
            ...
            <div class="bg-slate-800/50 px-10 py-6 border-b border-slate-700 flex justify-between items-center">
                <h3 class="text-2xl font-black uppercase text-white tracking-tighter italic">Edit <span class="text-blue-500">Layanan</span></h3>
                ...
                <div class="p-10 grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div class="space-y-8">
                        <div class="bg-slate-800/30 p-6 rounded-[2rem] border border-white/5">
                            <label class="block text-[10px] font-black uppercase text-blue-500 mb-6 tracking-[0.2em] text-center">Pratinjau Foto Layanan</label>
                            <div class="flex items-center justify-around gap-4 bg-slate-900/50 p-6 rounded-2xl border border-slate-700 shadow-inner">
                                <div class="text-center space-y-3">
                                    <div class="w-24 h-24 rounded-2xl border-2 border-slate-700 flex items-center justify-center bg-slate-800 overflow-hidden">
                                        <img id="edit_old_preview" src="" class="hidden w-full h-full object-cover">
                                        <i id="edit_old_placeholder" class="fa-solid fa-image text-slate-500 text-2xl hidden"></i>
                                    </div>
                                    <p class="text-[8px] font-black text-slate-500 uppercase">Lama</p>
                                </div>
                                <i class="fa-solid fa-right-long text-slate-700 text-xl"></i>
                                <div class="text-center space-y-3">
                                    <div class="w-24 h-24 rounded-2xl border-2 border-dashed border-blue-500/50 flex items-center justify-center bg-slate-800 overflow-hidden">
                                        <img id="edit_new_preview" src="" class="hidden w-full h-full object-cover">
                                        <i id="preview_edit_placeholder" class="fa-solid fa-cloud-arrow-up text-blue-500/30 text-2xl"></i>
                                    </div>
                                    <p class="text-[8px] font-black text-blue-500 uppercase">Baru</p>
                                </div>
                            </div>
                            <input type="file" name="gambar" class="mt-6 w-full bg-slate-800 border border-slate-700 px-6 py-3 rounded-2xl text-[10px] text-slate-400 file:mr-4 file:py-1 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-blue-600 file:text-white" onchange="previewFile(event, 'edit_new_preview', 'preview_edit_placeholder')">
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-[10px] font-black uppercase text-slate-500 mb-2 ml-1">Nama Layanan</label>
                                <input type="text" id="edit_nama" name="nama_layanan" required class="w-full bg-slate-800 border border-slate-700 px-6 py-4 rounded-2xl text-sm font-bold text-white outline-none focus:border-blue-500 transition-all">
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" id="edit_harga" name="harga" required class="w-full bg-slate-800 border border-slate-700 pl-14 pr-6 py-4 rounded-2xl text-sm font-bold text-white outline-none focus:border-blue-500 transition-all">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col">
                        <label class="block text-[10px] font-black uppercase text-blue-500 mb-4 ml-1 tracking-[0.2em]">Deskripsi Layanan</label>
                        <textarea id="edit_deskripsi" name="deskripsi" required class="flex-1 w-full bg-slate-800 border border-slate-700 px-8 py-8 rounded-[2.5rem] text-sm font-medium text-slate-300 leading-relaxed outline-none focus:border-blue-500 transition-all resize-none min-h-[350px] custom-scroll" placeholder="Tulis deskripsi detail..."></textarea>
                    </div>
                </div>

                <div class="bg-slate-800/30 px-10 py-8 border-t border-slate-700 flex justify-end">
                    <button type="submit" class="w-full md:w-auto bg-blue-600 hover:bg-blue-500 text-white px-16 py-5 rounded-2xl font-black uppercase text-xs tracking-[0.2em] shadow-xl active:scale-95 transition-all">
                        Update Data Layanan
                    </button>
                </div>
            </form>
    ...
    <div id="modalAdd" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg bg-slate-900 border border-slate-700 rounded-[2.5rem] shadow-2xl p-10">
            <h3 class="text-2xl font-black uppercase text-white mb-8 italic">Tambah <span class="text-blue-500">Layanan</span></h3>
            <form action="{{ route('admin.layanan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                <div><label class="block text-[10px] font-black uppercase text-slate-500 mb-2">Nama Layanan</label><input type="text" name="nama_layanan" required placeholder="Contoh: Deep Clean" class="w-full bg-slate-800 border border-slate-700 px-5 py-4 rounded-xl text-sm font-bold text-white outline-none focus:border-blue-500"></div>
                <div><label class="block text-[10px] font-black uppercase text-slate-500 mb-2">Deskripsi</label><textarea name="deskripsi" required class="w-full bg-slate-800 border border-slate-700 px-5 py-4 rounded-xl text-sm font-bold text-white h-24 resize-none outline-none focus:border-blue-500"></textarea></div>
                <div><label class="block text-[10px] font-black uppercase text-slate-500 mb-2">Harga (Tanpa Titik)</label><input type="number" name="harga" required placeholder="30000" class="w-full bg-slate-800 border border-slate-700 px-5 py-4 rounded-xl text-sm font-bold text-white outline-none focus:border-blue-500"></div>
                ...
                <div class="pt-4"><button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-lg">Simpan Layanan</button></div>
            </form>
        </div>
    </div>
    ...
    <script>
        function openEditModal(id, nama, deskripsi, harga, currentImage) {
            ...
            const oldPreview = document.getElementById('edit_old_preview');
            const oldPlaceholder = document.getElementById('edit_old_placeholder');
            ...
            let url = "{{ route('admin.layanan.update', ':id') }}";
